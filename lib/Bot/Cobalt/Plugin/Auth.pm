...
sub _init_superusers {
  ...
  ALL: {
    if (exists $su{'-ALL'}) {
      unless (ref $su{'-ALL'} eq 'HASH') {
        logger->error("Skipping -ALL; configuration is not a hash");
        last ALL
      }
    
      for my $user (keys %{$su{'-ALL'}}) {
        my $this_user = delete $su{'-ALL'}->{$user};    

        CONTEXT: for (keys %su) {
          next CONTEXT if $_ eq '-ALL';
          $su{$_}->{$user} = $this_user
        }
      } 

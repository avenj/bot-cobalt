...
automatically loaded at runtime unless B<NoAutoLoad> is a true value.
...
If an Opts: directive is supplied, it will be loaded under its own hash 
key; see L</PluginOpts>.
...
scalar, or it might be a reference of some sort, such as a hash:
  ...
  sub Bot_some_event {
    ...
    unless (ref $ev_hash eq 'HASH') {
      ...
      logger->warn("some_event expected a hashref");
...
Be cautious with your argument modifications; unless a handler eats the 
event (see L</"Returning proper values">, below), these changes are 
carried through to the next handler in the pipeline.
...
L<POE::Component::IRC>; thusly it is guaranteed to be available if 
L<Bot::Cobalt> is.

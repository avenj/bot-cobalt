...
B<The English langset is built in to this module at install time.>
...
Plugins can use this to read in a langset file, if they like:
  ...
  my $rpl_hash = Cobalt::Lang->load_langset('english', $prefix);
...
Events are emitted if load_langset is accessed via Cobalt::Core.

...
Cobalt::Lang -- A role for language set loading
...
to load their own langset, if you'd like -- just consume this role)

Langsets should be in a YAML format readable by L<YAML::XS>.
...
Plugins can use this to read in a langset file, if they like:
  ...
  my $rpl_hash = $self->load_langset('english', $prefix);
...
Events are emitted if load_langset is accessed via a Cobalt::Core 
instance.

...
Cobalt::Manual::PluginDist - packaging Cobalt plugins

=head1 DESCRIPTION

Packaging Cobalt plugins is just like packaging any other Perl module, 
except that you can (if you like) install them to the Cobalt::Plugin::User 
namespace.
...
See L<Cobalt::Manual::Plugins> for more on writing plugins.
...
Here's a simple one based around one centralized plugin:
  ...
  my $build = Module::Build->new(
    module_name   => "Cobalt::Plugin::User::MyPlugin",
    dist_abstract => "My Cobalt2 plugin",
...
You could distribute a plugin set with a B<Build.PL> along these lines:
  ...
  my $build = Module::Build->new(
    dist_version  => '0.01',
    module_name   => 'Cobalt::Plugin::User::MyPlugins',
    dist_name     => 'Cobalt-Plugin-User-MyPlugins',
    dist_abstract => 'My Cobalt2 plugin set',
    dist_author   => 'Me <myself@example.ext>',
    license => 'perl',
    create_makefile_pl => 'small',
    requires => {
...
Users of your plugin can install the distribution like so:

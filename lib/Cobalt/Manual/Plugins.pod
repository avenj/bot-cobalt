...
Plugins guide revision: 0.010

Last updated for cobalt 2.00_12
...
'cfg->{plugin_cf}->{$pkg}->{PluginOpts}' -- see L</PluginOpts>.
...
B<< $core->cfg->{plugin_cf} >>, keyed on package name:

  my $pkg = __PACKAGE__;
  my $plug_cf = $core->cfg->{plugin_cf}->{$pkg};
...
the timer pool directly via B<< $core->TimerPool->{TIMERS} >>, which is a 
...
B<warnings> pragmas and import the '5.12' featureset (see 'perldoc feature')
...
Cobalt::Core makes use of L<Moose> -- it's perfectly valid to use 
Moose for your plugins, as well (and it's already loaded!)
...
Moose is a proper object system for Perl, covering Perl's fairly 
basic OO facilities with a lot of tasty sugar.

Moose may make life much easier when writing large plugins requiring 
a useful inheritance structure.

You will probably want to read the L<Moose::Manual> if you are not 
previously familiar with Moose.

B<A reloadable plugin cannot call meta->make_immutable> -- attempting to 
reload a plugin that has been made immutable will break spectacularly.
Mark the plugin as NON_RELOADABLE or skip the immutable call.
...
B<Cobalt::Core> provides an easy object-oriented frontend to parsing RPLs 
via L<Cobalt::Utils/"rplprintf">.

A rpl_parser call creates a new B<Cobalt::RPL> object referencing the 
B<< $core->lang >> language set.

  ## Ready to parse some responses:
  my $rplp = $core->rpl_parser;

The only methods provided by the new object are B<new> and B<Format> -- 
all other methods calls set or retrieve an arbitrary variable's value:

  ## Set up a few vars for a response:
  $rplp->nick( $nickname );
  $rplp->chan( $channel );
  ## Retrieve a var, if needed:
  my $current_rpl_chan = $rplp->chan;

After variables are set up, passing a RPL key to B<Format> returns a 
C<rplprintf>-formatted string (or the RPL name if it is missing):

  my $resp = $rplp->Format("RPL_SOME_REPLY");

This interface is useful for situations where you have few variables to 
replace in templated RPL strings, but a large number of potential RPL 
responses to process. Sometimes it is more convenient to use C<rplprintf> 
directly.

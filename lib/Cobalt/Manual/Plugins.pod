...
Plugins guide revision: 0.013

Last updated for cobalt 2.00_18
...
B<< $core->cfg->{plugin_cf} >>, keyed on plugin alias:
  ...
  ## ask Core for our alias:
  my $plugin_alias = $core->PluginObjects->{$self};
  my $plug_cf = $core->cfg->{plugin_cf}->{$plugin_alias};
...
$self is generally fairly convenient:
  ...
  my $plug_cf = $self->{core}->get_plugin_cfg( $self );

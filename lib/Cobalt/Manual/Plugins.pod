...
For a quick-start guide, try L<Cobalt::Manual::Plugins::Tutorial>.
...
=head2 A basic plugin outline
  ...
  sub Bot_public_msg {
    ...
    ## Receives a Cobalt::IRC::Message::Public object:
    my $msg     = ${ $_[0] };
...
A _msg event is passed a context and a L<Cobalt::IRC::Message> object:
...
See L<Cobalt::IRC::Event>, L<Cobalt::IRC::Message> and 
L<Cobalt::IRC::Message::Public> for the complete 
...
L<Cobalt::Core> is actually an instanced singleton; it can be retrieved 
...
Each value will be a L<Cobalt::IRC::Server> object.
...
Returns the current L<Cobalt::Core> version.
...
Retrieves the appropriate L<Cobalt::IRC::Server> object from 
B<<$core->Servers>>.

A server context must be specified.

See L<Cobalt::IRC::Server> for details on methods that can be called 
...
Timer methods are provided by the L<Cobalt::Core::Role::Timers> role.
Each individual timer is a L<Cobalt::Timer> object; if you plan to 
...
L<Cobalt::Core::Role::Timers/timer_set>, so review the aforementioned 
...
(The L<Cobalt::Timer> object interface uses the B<alias> attribute.)
...
(The L<Cobalt::Timer> object interface uses the B<id> attribute.)
...
if needed; it will be a L<Cobalt::Timer> object:
...
Retrieve the L<Cobalt::Timer> for this active timer (or undef if not 
found).
...
The second argument is the removed L<Cobalt::Timer> object.
...
It may be necessary or convenient to use L<Cobalt::Core> methods from 
outside of a syndicated event handler.

L<Cobalt::Core> is an instanced singleton; loaded plugins can always 
retrieve the running L<Cobalt::Core> via the B<instance> method:

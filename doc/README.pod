...
=head2 Run-mode related options
...
=head2 Path-related options
   ...
   --etc=/path/to/etcdir 
     Specify a confdir. Defaults to EXECDIR/etc/ 
  
   --var=/path/to/vardir 
     Specify directory used for logs and databases
     Defaults to EXECDIR/var/
...
(See L</USAGE> for information on using a different C<etc/>)
